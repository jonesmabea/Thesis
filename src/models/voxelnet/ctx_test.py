...
if __name__ == '__main__':
    ...
    parser.add_argument('-exp', '--experiment', type=str, nargs='?', default="first",
			help='set comet ml experiment name')
    parser.add_argument('-ctx', '--context', type=str, nargs='?', default="urban", help="Enter context")
    args = parser.parse_args()
    dataset_dir = cfg.DATA_DIR
    val_dir = os.path.join(cfg.CONTEXT_DIR, args.context)
    save_model_dir = os.path.join('./save_model', args.tag)

    experiment = Experiment(api_key="xXtJguCo8yFdU7dpjEpo6YbHw",project_name=args.experiment)
