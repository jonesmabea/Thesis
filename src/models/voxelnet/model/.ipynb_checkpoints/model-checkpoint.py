...
class RPN3D(object):
    ...
    def predict_velo_step(self, session, data,vis=False):
        # input:
        #     (N) tag
        #     (N, N') label(can be empty)
        #     vox_feature
        ...
        for batch_id in range(len(self.avail_gpus) * self.single_batch_size):
            # remove box with low score
            ind = np.where(batch_probs[batch_id, :] >= 0.3)[0]
            tmp_boxes3d = batch_boxes3d[batch_id, ind, ...]
            tmp_boxes2d = batch_boxes2d[batch_id, ind, ...]
            tmp_scores = batch_probs[batch_id, ind]
        ...
        if vis:
            ...
            for i in range(len(tag)):
                ...
                base = "/mnt/storage/home/ja17618/scratch/Kitti/object/training/calib/"
                P, Tr, R = load_calib( os.path.join( base, '00'+cur_tag + '.txt' ) )
                ...
                bird_view = draw_lidar_box3d_on_birdview(bird_view, ret_box3d[i], ret_score[i],
                                                 np.array([]), factor=cfg.BV_LOG_FACTOR,P2=P, T_VELO_2_CAM=Tr, R_RECT_0=R)
            ...
            return tag, ret_box3d_score, bird_views, heatmaps
